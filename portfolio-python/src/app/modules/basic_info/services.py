...
class BasicInfoService:
    ...
    async def fetch_basic_info(self):
        ...
        if not basic_info:
            return BasicInfoResponse(success=False, message="Basic info not found", data=None)
        return BasicInfoResponse(success=True, message="Basic info fetched successfully", data=BasicInfoData(**basic_info))
    ...
    async def create_basic_info(self, name, email, phone, bio, about_me, github_url, linkedin_url, website_url, avatar_url, favicon_url):
        ...
        if not basic_info:
            return BasicInfoResponse(success=False, message="Failed to create basic info", data=None)
        return BasicInfoResponse(success=True, message="Basic info created successfully", data=BasicInfoData(**basic_info))

    async def update_basic_info(self, id, data):
        basic_info = await self.repository.update(id, data)
        if not basic_info:
            return BasicInfoResponse(success=False, message="Failed to update basic info or record not found", data=None)
        return BasicInfoResponse(success=True, message="Basic info updated successfully", data=BasicInfoData(**basic_info))

    async def delete_basic_info(self, id):
        basic_info = await self.repository.delete(id)
        if not basic_info:
            return BasicInfoResponse(success=False, message="Failed to delete basic info or record not found", data=None)
        return BasicInfoResponse(success=True, message="Basic info deleted successfully", data=None)